...
import {
	usePowerSupplyApiStore,
	usePowerSupplyStore,
} from '@/store/power-supply';
...
const PowerSupply = () => {
	...
	useEffect(() => {
		if (!isLoading && powerSupplies.length > 0) {
			const uniquePower = Array.from(
				new Set(powerSupplies.map(item => item.Power)),
			).sort((a, b) => {
			...
			const uniqueManufacturers = Array.from(
				new Set(powerSupplies.map(item => item.Manufacturer)),
			);
			setUniqueManufacturers(uniqueManufacturers);

			const minPriceRange = Math.min(
				...powerSupplies.map(item => parseInt(item.price, 10)),
			);
	...
	const { powerSupply, setPowerSupply, resetPowerSupply } =
		usePowerSupplyStore();
									...
									<div className="ml-2 relative w-full flex gap-4 h-12 items-center">
										<Autocomplete
											...
											onChange={(event, value) => {
												const selected = powerSupplies.find(
